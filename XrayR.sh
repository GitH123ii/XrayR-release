...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/GitH123ii/XrayR-release/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
#        return 0
#    fi
    bash <(curl -Ls https://raw.githubusercontent.com/GitH123ii/XrayR-release/master/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/XrayR -N --no-check-certificate https://raw.githubusercontent.com/GitH123ii/XrayR-release/master/XrayR.sh
    if [[ $? != 0 ]]; then
        echo ""
...
--- https://github.com/GitH123ii/XrayR ---
  ${green}0.${plain} 退出脚本
————————————————
